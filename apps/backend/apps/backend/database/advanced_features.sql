-- ZAPNINJA Database Advanced Features
-- Advanced indexing, views, and performance optimizations
-- Created: 2025-09-05

-- ==================================================
-- ADVANCED COMPOSITE INDEXES FOR PERFORMANCE
-- ==================================================
...
CREATE INDEX IF NOT EXISTS idx_whatsapp_sessions_active_only 
ON whatsapp_sessions(session_name, phone_number) 
WHERE is_active = true;

-- Index only active users
CREATE INDEX IF NOT EXISTS idx_whatsapp_users_active_only 
ON whatsapp_users(phone_number, name) 
WHERE is_active = true;

-- Index only non-expired context
CREATE INDEX IF NOT EXISTS idx_user_context_non_expired 
...
-- ==================================================
-- ADVANCED FUNCTIONS FOR ANALYTICS
-- ==================================================
...
BEGIN
  ...
  health_score := GREATEST(0, LEAST(100,
    ...
    -- Response time (25 points max) - penalty for slow responses
    CASE 
...
-- ==================================================
-- TRIGGER FOR AUTOMATIC CONTEXT CLEANUP
-- ==================================================

-- Function to automatically manage context relevance
CREATE OR REPLACE FUNCTION auto_manage_context_relevance()
RETURNS TRIGGER AS $$
BEGIN
  -- Decrease relevance score for old context when new context is added
  UPDATE user_context 
...
-- Create trigger for context management
CREATE TRIGGER trigger_auto_manage_context_relevance
  AFTER INSERT ON user_context
  FOR EACH ROW
  EXECUTE FUNCTION auto_manage_context_relevance();
...
/*
-- These functions can be called by cron jobs or scheduled tasks:

-- Daily cleanup (run at 2 AM)
SELECT cleanup_old_data(90);

-- Weekly optimization (run Sundays at 3 AM)
...
-- Final comment
COMMENT ON SCHEMA public IS 'ZAPNINJA Advanced Database Features - Performance optimized with analytics and monitoring capabilities';